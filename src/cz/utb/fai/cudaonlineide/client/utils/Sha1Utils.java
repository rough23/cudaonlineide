...
/**
 * Class provides sha1 hash from given text.
 * 
 * @author Belanec
 *
 */
public class Sha1Utils {
	...
	/**
	 * Method create hash from given text.
	 * 
	 * @param s Text to hash.
	 * @return Hash.
	 */
	public static native String getSha1Digest(String s) /*-{
		//
		...
		function rstr_hmac_sha1(key, data) {
			...
			var hash = binb_sha1(ipad.concat(rstr2binb(data)),
					512 + data.length * 8);
				...
				for (var j = 0; j < 80; j++) {
					if (j < 16)
						...
						w[j] = bit_rol(w[j - 3] ^ w[j - 8] ^ w[j - 14]
								^ w[j - 16], 1);
					var t = safe_add(safe_add(bit_rol(a, 5),
							sha1_ft(j, b, c, d)), safe_add(safe_add(e, w[j]),
							sha1_kt(j)));
					e = d;
					d = c;
					c = bit_rol(b, 30);
